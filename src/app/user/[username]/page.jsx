...
export default function PublicProfilePage() {
  ...
  const { copy, languagePreference } = useAuth();
  ...
  if (!data) return (
    <div className="min-h-dvh bg-[var(--bg-primary)]"><TopBar />
      <main className="max-w-lg mx-auto px-4 pt-20 text-center"><p className="text-[var(--text-secondary)]">{copy.search.noResults}</p></main>
      <BottomNav /></div>
  );
      ...
      <main className="max-w-lg mx-auto px-4 pt-6 pb-nav space-y-5">
        ...
        <div className="glass-card p-6 text-center space-y-3 animate-slide-up">
          ...
          <div className="flex items-center justify-center gap-3 text-xs text-[var(--text-muted)]">
            ...
            <span className="flex items-center gap-0.5"><Calendar size={10} /> {copy.leaderboard.joined} {timeAgo(user.joinedAt)}</span>
          </div>
        ...
        {judgeScore && (
          <div className="glass-card p-5 animate-slide-up" style={{ animationDelay: '80ms' }}>
            <div className="grid grid-cols-3 gap-3 text-center">
              <div><p className="text-2xl font-black gradient-text">{judgeScore.score.toFixed(1)}</p><p className="text-[10px] text-[var(--text-muted)] font-bold uppercase">{copy.leaderboard.scoreLabel}</p></div>
              <div><p className="text-2xl font-black">{judgeScore.totalVerdicts}</p><p className="text-[10px] text-[var(--text-muted)] font-bold uppercase">{copy.feed.verdicts}</p></div>
              <div><p className="text-2xl font-black text-[var(--accent-cyan)]">{judgeScore.fairRatings}</p><p className="text-[10px] text-[var(--text-muted)] font-bold uppercase">{copy.leaderboard.fair}</p></div>
            </div>
          </div>
        )}
        ...
        <div className="flex gap-2 animate-slide-up" style={{ animationDelay: '120ms' }}>
          <button onClick={() => setTab('verdicts')} className={`flex-1 flex items-center justify-center gap-1.5 py-3 rounded-xl text-xs font-bold transition-all ${tab === 'verdicts' ? 'bg-gradient-to-r from-[var(--accent-orange)] to-[var(--accent-pink)] text-white' : 'bg-[var(--bg-elevated)] text-[var(--text-muted)]'}`}>
            <MessageSquare size={14} /> {copy.feed.verdicts} ({recentVerdicts?.length || 0})
          </button>
          <button onClick={() => setTab('cases')} className={`flex-1 flex items-center justify-center gap-1.5 py-3 rounded-xl text-xs font-bold transition-all ${tab === 'cases' ? 'bg-gradient-to-r from-[var(--accent-purple)] to-[var(--accent-cyan)] text-white' : 'bg-[var(--bg-elevated)] text-[var(--text-muted)]'}`}>
            <Flame size={14} /> {copy.feed.cases} ({submittedCases?.length || 0})
          </button>
        </div>
        ...
        {tab === 'verdicts' && (
          <div className="space-y-2">
            {recentVerdicts?.length === 0 ? (
              <div className="glass-card p-8 text-center"><MessageSquare size={32} className="mx-auto text-[var(--text-muted)] mb-2" /><p className="text-sm text-[var(--text-secondary)]">{copy.verdict.noVerdicts}</p></div>
              ...
              <Link key={v.id} href={v.case ? `/case/${v.case.shareSlug}` : '#'} className="glass-card p-4 block space-y-2 hover:border-[var(--border-glow)] transition-all animate-slide-up active:scale-[0.98]" style={{ animationDelay: `${i * 50}ms` }}>
                ...
                <div className="flex items-start gap-2">
                  <span className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold shrink-0 ${SIDE_COLORS[v.side]}`}>{getSideLabel(v.side, languagePreference)}</span>
        ...
        {tab === 'cases' && (
          <div className="space-y-2">
            {submittedCases?.length === 0 ? (
              <div className="glass-card p-8 text-center"><Flame size={32} className="mx-auto text-[var(--text-muted)] mb-2" /><p className="text-sm text-[var(--text-secondary)]">{copy.profile.noCasesSubmitted}</p></div>
              ...
              <Link key={c._id} href={`/case/${c.shareSlug}`} className="glass-card p-4 block space-y-2 hover:border-[var(--border-glow)] transition-all animate-slide-up active:scale-[0.98]" style={{ animationDelay: `${i * 50}ms` }}>
                ...
                <div className="flex items-center gap-3 text-xs text-[var(--text-muted)]"><span className="flex items-center gap-1"><MessageSquare size={10} /> {formatCount(c.voteCount)} {copy.feed.votes}</span>{c.city && <span className="flex items-center gap-0.5"><MapPin size={10} /> {c.city}</span>}</div>
              </Link>
            ))}
          </div>
        )}
